...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      // Step 1: Check the status of the generation request
      ...
      if (!statusResponse.ok) {
        ...
        console.error(`Error checking status: ${statusResponse.status}, ${errorData}`);
      ...
      res.status(500).json({ error: error.message || 'Error checking status. Please try again later.' });
    }
  } else {
